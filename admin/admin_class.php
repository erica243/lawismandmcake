...
Class Action {
    ...
    function login2() {
        // Ensure the POST request contains 'recaptcha_token'
        extract($_POST);
        ...
        if (empty($recaptcha_response)) {
            ...
            error_log("reCAPTCHA token received: " . $recaptcha_response);
        }
        ...
        if (curl_errno($ch)) {
            ...
            return json_encode(['status' => 'error', 'message' => 'reCAPTCHA verification failed due to server error.']);
        }
    
        // Close cURL session
        curl_close($ch);
        
        // Decode the reCAPTCHA response
        $response_data = json_decode($verify, true);
    
        // Log the full reCAPTCHA response for debugging
        error_log("reCAPTCHA verification response: " . print_r($response_data, true));
        ...
        // Define max failed attempts and lockout time
        $max_attempts = 3;
        $lockout_time = 10;
        ...
        if ($qry->num_rows > 0) {
            ...
            if ($is_verified) {
                ...
                // Update cart table with user ID (this assumes user is logged in)
