   ...
   function signup() {
    ...
    function forgot_password() {
        ...
        if ($result->num_rows === 0) {
            return json_encode(['status' => 'error', 'message' => 'Email not found in our records.']);
        }
        ...
        try {
            ...
            $mail->send();
            return json_encode(['status' => 'success', 'message' => 'An OTP has been sent to your email.']);
        } catch (Exception $e) {
            return json_encode(['status' => 'error', 'message' => 'Failed to send email: ' . $mail->ErrorInfo]);
        }
    }
